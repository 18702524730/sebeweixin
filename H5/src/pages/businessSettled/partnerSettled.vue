<template>
  <div class="business_settled">
    <div class="top_img">
      ...
      <img src="~assets/img/businessSettled/p_title.png" class="banner_tit" alt="">
      <p class="join">立即加入万亿级知识产权服务与交易市场</p>
    </div>

    <div class="tab_box" :class="{'right': activeIndex==1}" >
      <p class="item" @click="activeIndex=0">机构</p>
      <p class="item" @click="activeIndex=1">个人</p>
    </div>
    <div class="info_box" :class="{'info_box_rig': activeIndex==1}">
      ...
      <div class="line">
        <span class="key key_w">推荐人账号</span>
        <input type="text" class="value" placeholder="请填写推荐人账号" maxlength="11" v-model="tableInfo.inviteCode">
      </div>
      <div class="line line_text" v-if="activeIndex==0">
        <span class="key">机构介绍</span>
        <span class="value">{{ tableInfo.remark.length }}/200字</span>
        <!-- <input type="text" class="value_text" v-model="tableInfo.remark" maxlength="200" placeholder="请填写机构介绍"> -->
      ...
      <button class="join_btn" @click="submitFn">立即申请</button>
    ...
    <div class="platform_adwantage">
      ...
      <div class="a_item">
        <img src="~assets/img/businessSettled/p_1.png" alt="" class="img">
        <div class="text_box">
          <p class="title">全面服务</p>
          <p class="info">提供全生命周期服务</p>
        </div>
      </div>
      <div class="a_item">
        <img src="~assets/img/businessSettled/p_2.png" alt="" class="img">
        <div class="text_box">
          <p class="title">专业保障</p>
          <p class="info">全天候服务保证</p>
        </div>
      </div>
      <div class="a_item">
        <img src="~assets/img/businessSettled/p_3.png" alt="" class="img">
        <div class="text_box">
          <p class="title">定制服务</p>
          <p class="info">提供定制保护方案</p>
        </div>
      </div>
      <div class="a_item">
        <img src="~assets/img/businessSettled/p_4.png" alt="" class="img">
        <div class="text_box">
          <p class="title">智能平台</p>
          <p class="info">随时监控服务流程</p>
        </div>
      </div>
      <div class="a_item">
        <img src="~assets/img/businessSettled/p_5.png" alt="" class="img">
        <div class="text_box">
          <p class="title">高额收益</p>
          <p class="info">高比例利润分成</p>
        </div>
      </div>
    </div>
      ...
      methods: {
        ...
        checkPhoneFn () {
          ...
          .then( (res) => {
            ...
            if (res.data.channelApplyExist) {
              this.$message({
                type: 'warning',
                message: '您已入驻合伙人，请勿重复申请'
              })
              return
            }
            // if (res.data.spExists) {
            //   this.$vux.toast.text('您已入驻合伙人，请勿重复申请','top')
            //   return
            // }
            // if (res.data.applyExist) {
            //   this.$vux.toast.text('请勿重复提交。您提交的申请正在审核中，请耐心等待。如有疑问请咨询平台客服电话：0571-28253721', 'top')
            //   return
            // }
          })
        },
        ...
        submitFn () {
          if (this.activeIndex==0 && !this.tableInfo.companyName) {
            this.$vux.toast.text('请输入公司名称', 'top');
            return
          }
          if (this.activeIndex==0 && this.tableInfo.companyName.length>30 || this.activeIndex==0 && this.tableInfo.companyName.length<2) {
            this.$vux.toast.text('机构名称长度必须在2-30字范围内', 'top');
            return
          }
          ...
          if (this.tableInfo.name.length>30 || this.tableInfo.name.length<2) {
            this.$vux.toast.text('姓名长度必须在2-30字范围内', 'top');
            return
          }
          ...
          if (this.tableInfo.inviteCode && !/^1[0-9]{10}$/.test(this.tableInfo.inviteCode)) {
            this.$vux.toast.text('请输入正确格式的推荐人账号', 'top');
            return
          }
          ...
          if (this.activeIndex==0) {
            data.applyType = 3;  //机构渠道商
          }
          else if (this.activeIndex==1) {
            data.applyType = 2;  //个人渠道商
...
<style scoped lang='scss'>
  ...
  .business_settled {
    ...
    .weui-icon-success {color: linear-gradient(135deg,rgba(16,174,255,1),rgba(77,160,255,1)) !important; }
    background: url(~assets/img/businessSettled/p_bg.png) left top no-repeat #F5F6F9; background-size: 100%; width: 100%; height: 100%; padding: px(50) 0 px(30) 0;
    .top_img {
      width:100%; text-align: center;
      .logo {width: px(160); display: block; margin: 0 auto;}
      .banner_tit {width: px(360); display: block; margin: px(40) auto px(24) auto;}
      .join {width:px(504); margin:0 auto; text-align: center; color: #fff; font-size: px(26); text-align: center; margin-bottom: px(40);}
    }
    .tab_box {
    ...
    .platform_adwantage {
      ...
      .a_item {
        margin-left: px(87); display: flex; justify-content:flex-start; align-items:center; height: px(140); margin-bottom: px(60);
        .img {width: px(180); height: px(140);}
        .text_box {
          width: px(300); margin-left: px(60);
          .title {font-size: px(34); color: #3DB1FA;}
