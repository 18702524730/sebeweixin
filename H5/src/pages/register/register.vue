<template>
  <div class="page">
    ...
    <button  class="next-btn" v-show="isRegister" @click="goNext">{{next_msg}}</button>
...
<script>
  ...
  export default {
      ...
      methods: {
        ...
        getCapcha () {
          var self = this;
          self.$http.post(userUrl + '/api/member/register/capcha', qs.stringify({account: self.postData.phone}))
            .then((resp) => {
              self.timeout();
              // var data = resp;
              self.ver_tip = '验证码已发送';
        ...
        checkPassWord () {
          if( !this.postData.password.trim()) {
            this.ver_tip = '请设置密码';
            this.checkPostData = true;
            return false
          } else if(!/^(?![A-Z]+$)(?![a-z]+$)(?!\d+$)\S{8,16}$/.test(this.postData.password)){
              this.ver_tip = '密码由8-16位字符组成,且须包含两种字符';
              this.checkPostDatas = true;
              this.toastWidth = '12rem';
              return false;
          } else {
            return true
          }
        ...
        prevLogin () {
            ...
            self.$http.post(userUrl + '/api/beforeLogin', {
              ...
              var password = md5(self.postData.password);
              self.login(password);
            }).catch((error) => {
              self.submitDisabled = true;
              this.loading = false;
