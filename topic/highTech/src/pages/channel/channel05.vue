<template>
 <div class="channleWrap channelWarp3" :class="{'image_size':sizeImg}">
   <!-- <img class="sebe_image" src="~assets/img/channel/sebe.png" alt=""> -->
    ...
    <p class="companyName" :class="state ? 'animateBtn' : ''">杭州拾贝知识产权服务有限公司</p>
    <div class="boxForm" :class="formState ? 'animateForm' : ''">
      <div class="closeForm" @click="formState = false">x</div>
      <div class="formItem"><input type='text' :maxlength="30" v-model="name" placeholder="*请输入您的姓名"/></div>
      <div class="formItem"><input type='text' :maxlength="11" v-model="phone" placeholder="*请输入电话号码" /></div>
      <div class="formItem"><input type='text' :maxlength="30" v-model="remark" placeholder="*请输入您所擅长领域" /></div>
      <a @click="handleJoin">立即加入</a>
      <p v-show="errorTextState" class="errorText">{{errorText}}</p>
    </div>
    <div @click="formState = false" :class="['mask', formState ? 'maskShow' : '']"></div>
    <div class="confirm" :class="confirmState ? 'animateForm' : ''">
      <div class="closeForm" @click="confirmState = false">x</div>
      <p>感谢您对拾贝的支持</p>
      <p>我们将尽快与您取得联系！</p>
      <a class="confirmOk" @click="confirmState = false">确定</a>
    </div>
 </div>
</template>
<script>
...
export default {
   ...
   watch: {
    //  name (val) {
    //    if (this.validPhone(this.phone) && val && this.remark) {
    //      this.errorTextState = false
    //    } else {
    //      this.errorTextState = true
    //    }
    ...
    //  phone (val) {
		...
		// 		this.errorTextState = false
		// 	}
    //   if (this.name && this.validPhone(val) && this.remark) {
    //     this.btnState = true
    //   } else {
    //      this.btnState = false
    //    }
    //  },
    //  remark (val) {
    //   if (this.validPhone(this.phone) && val && this.name) {
    //      this.btnState = true
    //    } else {
    //      this.btnState = false
    //    }
...
.channleWrap{
  .companyName{font-size: 24px;line-height: 26px;color: #07d09e;width: 690px;position: static;margin-top: 15px;opacity: 0;
  ...
  .boxForm{background: #fff;border-top-left-radius: 10px;border-top-right-radius: 10px;position: absolute;left: 0;bottom: -570px;width: 100%;
    padding: 70px 0 50px;font-size: 30px;transition: bottom 0.5s;
