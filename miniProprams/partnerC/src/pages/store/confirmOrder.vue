<template>
 <div class="confirmOrder">
   <div style="height: 20rpx;background: #f8f8f8"></div>
   <div class="orderList">
    <h2 class="storeName" v-if="store">
  ...
  methods: {
    async getProductDetail() {
      ...
      this.productDetail.salePrice
        ? (this.total = (
            Number(this.productDetail.salePrice) * this.num
          ).toFixed(2))
        : "";
    },
    async creatPartnerOrder() {
      try{
        ...
        let data = await this.API.confirmOrder.creatPartnerOrder({
          id: this.productDetail.id,
          codes: this.productDetail.codes,
          salePrice: this.productDetail.salePrice,
          serviceNumber: Number(this.num)
        });
        this.preventBtnStatus = false;
        this.wechatPayFn(data);
      }catch (err) {
        this.preventBtnStatus = false;
      }
    ...
    wechatPayFn(orderData) {
      let self = this;
      var paymentBody = {};
      paymentBody.order_sn = orderData.orderFictitiousSn;
