 ...
 <div class="searchWrap">
   <div class="searchPanel">
     <i class="icon iconfont icon-sousuo"></i>
      ...
      <input type="search" :focus="true" placeholder="服务搜索" placeholder-class="phcolor" @confirm="searchAllProduct" v-model="keyword" />
...
export default {
  ...
  methods: {
    ...
    async searchHistory () {
      let data = await this.API.search.searchHistory({type: 2})
      if (data) {
    ...
    clearAll () {
      this.API.search.removeSearchHistory({type: 2}).then(() => { this.searchHistory({type: 2}) })
    },
    clearInput () {
      this.keyword = ''
    },
