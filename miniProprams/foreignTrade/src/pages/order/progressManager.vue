...
<div class="manage_progress">
  ...
  <div class="info_progress">
    <div class="mt5" v-for="(item,$index) in progressList" :key="$index" :class="{'progress':$index == 0}">
  ...
  <div class="fixed-bottom row-center bottomBar">
    <span class="lc del" @click="back">返回服务单详情</span>
  </div>
</div>
</template>
<script>
  ...
  methods: {
    ...
    back(){//返回上一页
      this.$storage.remove('manageInfo');
      this.$router.go(-1);
    },
    async lookFile(url){
      console.log(url)
  ...
  mounted(){
    ...
    console.log(this.manageInfo)
    this.search()
  },
  computed:{
    payMoney:{
      get () {
        return this.manageInfo.payMoney = Filters.filters.priceformat(this.manageInfo.payMoney || 0);
      },
      set (value) {
      }
    }
...
border-radius:5px;padding:30px;
  ...
  .bottomBar{width:100%;height:118px;text-align:center;background-color:#f8f8f8;padding-top:20px;
    span{font-size:32px;color:#2F80F6;line-height:98px;display:inline-block;width:100%;background-color:#fff;}
  }
}
</style>
