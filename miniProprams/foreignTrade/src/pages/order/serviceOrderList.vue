...
<div class="list_info">
  <div class="tab weui-flex" :class="{'boxShow':scrollY}">
    ...
    <div class="weui-flex__item mr30" :class="{'bdc':feedbackState == 1}" @click="feedbackStateChange(1)">待服务</div>
    <div class="weui-flex__item mr30" :class="{'bdc':feedbackState == 2}" @click="feedbackStateChange(2)">服务中</div>
    <div class="weui-flex__item mr30" :class="{'bdc':feedbackState == 3}" @click="feedbackStateChange(3)">服务完成</div>
  </div>
  <div style="height:47px;" v-if="dataLists"></div>
  ...
  <!-- 其他服务状态 -->
    <div v-for="(item,index) in dataLists" :key="index" @click="toServiceDetail(item)">
      <div class="single_info" :class="{'mt20' : index == 0}" style="box-sizing:border-box;">
        ...
        <!-- <div class="row-end info_num weui-flex mt25 mr20" v-if="item.orderServiceState==2||item.orderServiceState==3">
...
<script>
  ...
  mounted(){
    this.feedbackState = this.$root.$mp.query.feedbackState || 0;
    this.search()
  },
  computed:{
    dataListInfo:{
...
.list_info .single_info .info_num{
  button.reject{color:#666;}
}
</style>
