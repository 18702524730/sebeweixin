...
//   console.log('testquery', query, pages)
//   let url = '/pages/authorize/authorize';
//   console.log('qs', url + '?' + qs.stringify(query))
...
const rMethod = (url = '', data = {}, flyConfig = {}, defaultTipConfig = {}) => {
  ...
  return flyio.then(res => {
    ...
    // if (res && (res.code == '0800001' || res.status == 401)) {
    //   goLogin();
    // }
    // setTimeout(() => {
    ...
    // console.log('err:', err.engine.responseURL)
    // let res = err.response && err.response.data;
    // if (res && (res.code == '0800001' || (res.status == 401 ))) {
    //   goLogin();
    // }
    // console.log('err2:', err.engine.responseURL)
    tipConfig.isLoading && Config.loading.loadingHide()
    !tipConfig.isLoading && wx.hideNavigationBarLoading()
