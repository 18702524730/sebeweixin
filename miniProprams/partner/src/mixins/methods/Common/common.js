...
export default {
  ...
  async checkLoginAndMark(markFrom) {
    ...
    if (sessionRes.isLogin) {
      ...
      console.log('zzquery', query)
      // 如果是珍珠页面则不跳转至开店页面直接下一步跳转至珍珠页面 iptType珍珠页面标识
      if (!userData.isHaveStore&&!query.iptType) {
        this.$router.replace({
      ...
      if (!markFrom) {
        if (query.path) {
          wx.reLaunch({
            url: query.path + '?' + qs.stringify(query),
            fail: error => { wx.switchTab({ url: indexPath}) }
          });
          return;
        }
  ...
  /**
   * 用于mpvue重置state，初始化数据
   * @param {[]} arr 需要充值的数组变量 
   * @param {*} options 配置对象用于重置_options字典
   */
