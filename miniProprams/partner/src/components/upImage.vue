  ...
  export default{
    ...
    methods :{
      ...
      async chooseImg (type) {
        // 面板选择动作标志
        this.$storage.set('isChoose', true);
        let url = await this.API.common.uploadImg(type, this.limit, this.limitWid, this.limitHei);
      ...
      doClick(){
        setTimeout(()=>{
          this.$storage.set('isChoose', true);
          this.$emit('doClick', this.imgUrl, this.valueName, this.fileName);
        }, 100)
      }
    },
    mounted(){
