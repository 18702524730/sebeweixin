<template>
  <div class="noticeWrap">
    <div class="noticeItem" v-for="(item, index) in noticeList" :key="index" @click="$router.push('/pages/index/detail?id=' + item.id)">
...
export default {
  ...
  onShareAppMessage() {
    ...
    obj = {
      ...
      path: "/pages/authorize/authorize?memberId="+ memberId + "&shareId=7",
  ...
  methods: {
    async init (type) {
      console.log(filter)
      let data = await this.API.homePage.indexList({ pgCount: this.pgCount, pgIndex: this.pgIndex, category: 5 })
      data['list'].map(item => {
