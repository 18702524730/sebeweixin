  ...
  <div class="bindPhone">
    ...
    <div class="innerBox">
      ...
      <div class='register'>
        ...
        <button :class="['login', isHight ? 'hight' : '', isIpt ? 'ipt' : '']" @click="goLogin">{{loginText}}</button>
      </div>

    </div>
    <!-- <steps v-if="!isIpt" :step="1"></steps> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeColor() {
        this.color = this.color === '#ddd' ? '#2F80F6' : '#ddd';
      },
      ...
      async goLogin() {
        ...
        try {
          ...
          let query = this.$root.$mp.query;
          // query.scene ? query.scene = decodeURIComponent(query.scene) : ''
          let memberId;
          let appType = 'b'; // 判断c端还是B端过来的二维码，默认B端
          if (query.scene) {
            ...
            if (query.scene.indexOf('memberId=') != -1) {
              ...
              // memberId = query.scene;
            }
          } else {
            // 分享给好友
            memberId = query.memberId;
          }
          console.log('memberId_type', memberId, appType)
          let loginRes = {};
          if (appType == 'c') { // c端邀请的没有红包，不传邀请人id
            loginRes = await interfaces.login(this.postData.phone, this.postData.token);
          } else {
            loginRes = await interfaces.login(this.postData.phone, this.postData.token, memberId);
          }
          ...
          if(loginRes.code == 0) {
            ...
            if(appType == 'c'){
              // C端邀请，增加珍珠
              this.API.homePage.invitedIpt({memberId});
            }
      ...
      async openShop() {
        let storeCode = await this.API.interfaces.openShop();
      ...
      async loginSuccessHandle(){
        ...
        if (from) {
          ...
          // 如果是珍珠页面则不去开店页面，直接跳转至珍珠页面
          if (query.iptType) {
            path = '/pages/my/my_IPT'
          } else {
            await this.openShop()
            return
          }
  ...
  .bindPhone{
    ...
    .innerBox{
      ...
      .register{
        ...
        p{
          width: 600px;height: 90px;padding: 4px;border-radius:45px;border:1px solid #ddd;position: relative;font-size:30px;
        }
        .tel{
