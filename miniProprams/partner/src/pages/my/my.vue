  ...
  <div class="bindPhone">
    ...
    <div class='register'>
      ...
      <p class='identifying-code'>
        <input type="text"  maxlength="6" v-model="postData.token" placeholder="动态密码">
        <span :class=" preventCode ? 'code' : 'code disabled' "  @click.stop="getCode">{{codeText}}</span>
      </p>
      <button class="login" @click="goLogin">{{loginText}}</button>
    </div>
    <!-- <p class="remind">温馨提示，未注册拾贝网账号的手机号，登录时将自动注册，且代表您已同意<span @click="goToRule">《拾贝网用户注册协议》</span></p> -->
      ...
      async goLogin() {
        ...
        try {
          ...
          if(loginRes.code == 0) {
            ...
            this.$router.replace('/pages/authorize/openShop')
            /*wepy.switchTab({
...
<style lang='scss'>
  .bindPhone{
    h3{height:56px;line-height: 56px;font-size:40px;text-align: center;margin-top: 110px;font-weight:600;}
    .register{
      ...
      .tel{
        input{width: 100%;height: 100%;border-radius:50%;padding-left: 36px;color:#BBB;}
        button{padding: 0; border: 0;position: absolute;right: 40px;top:20px;color: #2F80F6;z-index: 2;}
      }
      .identifying-code{
        margin-top: 20px;
        input{width: 100%;height: 100%;border-radius:50%;padding-left: 36px;color:#BBB;}
