<template>
  <div class="setmail_page">
    <div class="line_wrap">
      <div class="line_item">
        <input type="text" class="lef" v-model="email" placeholder="请输入您邮箱地址" >
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      async saveFn () {
        if ( !Filter.validator.isMail(this.email)) {
          this.tips('邮箱格式错误');
          return
        }
        let resp = await this.API.my.savePersonInfo({email: this.email});
        this.$router.go(-1);
      }
    },
    onLoad () {
      this.email = this.$route.query.des;
    },
  }
</script>
