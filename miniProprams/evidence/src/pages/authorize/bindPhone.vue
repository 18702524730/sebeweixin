    ...
    <div class="innerBox">
      ...
      <div class='register'>
        ...
        <div class="remind" @click="changeColor">
          ...
          <div>
            <i>我已阅读并同意</i>
            <span @click.stop="goToRule">《拾贝网用户注册协议》</span>
          </div>
        </div>
        <button class="login" :class="{'hight': isHight}" @click="goLogin">{{loginText}}</button>
        <button class="closeBd" @click="closeFn">取消</button>
      </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      let sessionRes = await this.checkLoginAndMark();
      console.log('已授权未绑定sessionRes:', sessionRes)
    },
    methods: {
      tips (msg){
      ...
      async goLogin() {
        ...
        try {
          ...
          let loginRes = await interfaces.login(this.postData.phone, this.postData.token);
          if(loginRes.code == 0) {
      ...
      loginSuccessHandle(){
        ...
        console.log('bindPhoneQuery', query)
        let from = query.path;
        ...
        console.log('bindPhonePath', path)
        if (path.indexOf('index') != -1) {
          // 未登录过C端时，记录从B端带过来的storeCode
...
<style lang='scss'>
  .bindPhone{
    ...
    .innerBox{
      position: absolute;top: 30px;left: 40px;right: 40px;bottom: 30px;padding-top: 90px;box-shadow:0px 0px 20px 0px rgba(221,221,221,0.5);border-radius:6px;background: #fff url(~assets/img/authorize/bind_phone_bg.png) center bottom no-repeat;background-size: 100%;
      h3{height:56px;line-height: 56px;font-size:40px;text-align: center;font-weight:600;}
      .register{
        width: 600px;margin: 70px auto 0;
        p{
        ...
        .remind{
          margin-top: 30px;font-size:24px;color: #999;vertical-align: top;text-align: center;
        ...
        .hight{opacity:1;}
        .closeBd{
          width:600px;padding: 29px 0;text-align: center;
          border-radius:45px;font-size:32px;line-height: 1;border:1px solid rgba(0,150,255,1);
          margin-top: 30px;
          background: #fff;
