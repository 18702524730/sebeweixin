  ...
  <div class="certificate_detail">
    <div class="images">
      <scroll-view class="imgList" scroll-x :class="{'maxWidth':detailsInfo.egzCzhUrl&&detailsInfo.zjlCzhUrl}">
        ...
        <img v-if="detailsInfo.egzCzhUrl" @click="upImage(detailsInfo.egzCzhUrl)"  class="ml40" src="~assets/img/deposit/groupGzs.f49ea5b.png" alt="">
    ...
    <scroll-view @touchstart="touchStart" @touchend="touchEnd"  class="infoList" :class="{'maxHeight':bolHeight,'minHeight':!bolHeight}" scroll-y :scroll-top="firstTop">
      <div class="data_lists">
        ...
        <p>存证文件：<span class="name">{{detailsInfo.fileName}}</span><span class="look_img" @click="lookFileImage">查看</span></p> 
        ...
        <p>作品名称：{{detailsInfo.productionName}}</p>
        <p>权利取得：原始取得</p>
        <p>创作时间：{{productionTime}}</p>
        <p>发表状态：{{publishState}}</p>
        <p>存证申请时间：{{createTime}}</p>
        <p style="margin-bottom: 0;">存证完成时间：{{updateTime}}</p>
      </div>
    </scroll-view>
...
export default {
  ...
  computed:{
    productionTime:{
      ...
      get () {
        return Filters.filters.dateFormat(this.detailsInfo.productionTime,'date');
      },
      set (value) {
      }
    },
    ...
    publishState(){
      return Filters.filters.publishQy(this.detailsInfo.publishState);
    },
    notarizationPurpose(){
...
.certificate_detail{position: absolute;top:0;left:0;right:0;bottom:0;background-color:#f8f8f8;
  ...
  .imgList{overflow-x: scroll;height:750px;margin-top: 36px;white-space: nowrap;
  ...
  .defaultImage{overflow-x: scroll;height:750px;margin-top: 36px;white-space: nowrap;text-align:center;
    img{width:530px;height:750px;}
  }
  ::-webkit-scrollbar {
    width: 0;
    height: 0;
  ...
  .maxHeight{height:866px;
    .data_lists{top:386px;}
  }
  .minHeight{height:456px;
    .data_lists{top:46px;}
  }

  .infoList{position: absolute;bottom:0;z-index: 20;font-size:0;
    img{width: 312px;height: 46px;position: absolute;top:-44px;left:50%;margin-left: -156px;}
    .data_lists{background-color:#fff;width:690px;border-radius:20px 20px 0 0;box-shadow:0 2px 6px 0 rgba(0,0,0,0.07);margin:0 auto 0;padding-left: 50px;height:820px;position: absolute;left:0;right:0;bottom:0;padding-top: 60px;
      p{font-size:28px;color:#666;line-height:100%;margin-bottom: 42px;width:570px;position: relative;text-overflow:ellipsis;white-space :nowrap;
