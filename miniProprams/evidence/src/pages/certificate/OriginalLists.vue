<template>
  <div class="web_certificate">
    <scroll-view :scroll-y="true" upper-threshold class="lists">
      <div class="web_list" v-if="czLists&&czLists.length" v-for="item in czLists" :key="item.orderSn" @click="toDetails(item)">
        <div class="row-between name">
          <p class="name_l">订单号：{{item.orderSn}}</p>
          <p class="name_r">{{item.state==1?'存正中':item.state==2?'存证失败':item.state==3?'存证成功':'存证中'}}</p>
        </div>
        <div class="info row-between">
          <img src="~assets/img/deposit/fileImage.png" alt="">
          <div>
            <p class="url">{{item.fileName}}</p>
...
export default {
  ...
  onReachBottom() {
    ...
    console.log('上拉加载',this.pgIndex,pageAll)
  ...
  methods:{
    toDetails(item){
      this.$router.push({path: '/pages/certificate/certificateDetail?order_sn='+item.orderSn});
    },
    async search(){
      let self = this;
      let data = {
...
.web_certificate{position: relative;top:0;left:0;right:0;bottom:0;background-color:#f8f8f8;height:100%;
  ...
  .web_list{width:690px;min-height:242px;border-radius:20px;box-shadow:0px 2px 4px 0px rgba(210,210,210,0.5);background-color:#fff;margin:20px auto 0;
    .name{border-bottom:1px solid #eee;
      p{width:100%;line-height:50px;height:50px;font-size:20px;color:#333;}
      .name_l{text-align:left;padding-left: 30px;}
      .name_r{text-align: right;padding-right: 30px;}
    }
